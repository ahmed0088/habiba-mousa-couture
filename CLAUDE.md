...
**Staff onboarding looks up the person's existing client account instead of requiring a manual Firebase Console step.** The admin dashboard's "Add Staff Member" form searches the `clients` collection (populated automatically when someone signs up on the public site) by the typed email; if a match is found, it creates the `staff/{uid}` doc directly using that client's real Auth UID — no Firebase Console step needed. If no matching `clients` doc exists, the person must sign up on the public site first (client code still can't create Auth users directly — there's no Admin SDK/Cloud Function in this project). `staff_pending` is legacy/unused now that this lookup exists.
